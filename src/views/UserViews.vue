  ...
  <div class="home">
    ...
    >
      ...
      >
        ...
        <img 
          :src="`http://127.0.0.1:5000/uploads/${user.image_path}`" 
          alt="Imagen usuario" 
...
export default {
  ...
  methods: {
    ...
    async fetchUsers() {
      try {
        const response = await fetch('http://127.0.0.1:5000/user/lista_usuarios');
    ...
    async createUser() {
      ...
      try {
        const response = await fetch('http://127.0.0.1:5000/user/create-users', {
          method: 'POST',
          body: formData
        });
